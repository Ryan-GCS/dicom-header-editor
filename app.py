...
def compare_dicom(ds_a, ds_b) -> pd.DataFrame:
    ...
    for tag in all_tags:
        ...
        if in_a and in_b:
            ...
            kw, vr, va, vb, status = (
                dict_a[tag]["Keyword"], dict_a[tag]["VR"],
                dict_a[tag]["Value"], "", "only_a"
            )
        else:
            kw, vr, va, vb, status = (
                dict_b[tag]["Keyword"], dict_b[tag]["VR"],
...
if not st.session_state.phi_confirmed_once:
    ...
    <div style="
        ...
        font-size:12px; color:#00c864; margin-bottom:12px;
    ...
    if "Single" in upload_mode:
        ...
        if uploaded:
            if uploaded.name != st.session_state.filename:
                ...
                with st.expander(
                    f"📂 {len(file_list)} files in ZIP", expanded=False
                ):
                    st.dataframe(
                        pd.DataFrame({"File": file_list}), hide_index=True
                    )
                ...
                with zipfile.ZipFile(io.BytesIO(zb)) as zf:
                    for fname in zf.namelist():
                        ...
                        fb = zf.read(fname)
                        try:
                            ds_test = pydicom.dcmread(
                                io.BytesIO(fb), force=True
                            )
                            if len(ds_test) >= 3:
                                first_dcm_bytes = fb
                                break
    ...
    if st.session_state.ds is not None:
        ...
        with col1:
            search = st.text_input(
                "🔍 Search Tags",
                placeholder="e.g. PatientName or 0010,0010"
            )
        with col2:
        ...
        with col_a:
            ...
            selected_tag = (
                selected_option.split("  ")[0] if selected_option else None
            )
        with col_b:
            if selected_tag:
...
# ══════════════════════════════════════════════════════
    ...
    <div class="step-card" style="margin-bottom:24px;">
      <div class="step-header" style="gap:14px;">
        ...
        <div>
          <p class="step-title" style="margin-bottom:4px;">
    ...
    with col_a:
        st.markdown(
            '<div class="file-header-a">📄 &nbsp;<b style="color:#00d4ff;">'
            'File A</b> &nbsp; Reference / Source</div>',
            unsafe_allow_html=True,
        )
    ...
    with col_b:
        st.markdown(
            '<div class="file-header-b">📄 &nbsp;<b style="color:#9c27b0;">'
            'File B</b> &nbsp; Target / Modified</div>',
            unsafe_allow_html=True,
        )
    ...
    if st.session_state.cmp_df is not None:
        ...
        if n_issues == 0:
            ...
            <div class="diff-banner-ok">
                ...
                <div style="font-size:13px;color:#8892a4;">
                    All {n_total} tags match perfectly.
                </div>
            </div>
            """, unsafe_allow_html=True)
        else:
        ...
        df_view = df_full[
            df_full["Status"].isin(status_map[show_opt])
        ].copy()
        ...
        st.caption(
            ...
            "and apply to File B. Pixel Data (7FE0,0010) is protected."
        )
        ...
        if editable_df.empty:
            st.info("🎉 No differences — files are identical!")
        else:
            for _, row in editable_df.iterrows():
                ...
                with st.expander(label, expanded=False):
                    ...
                    with e2:
                        current_staged = st.session_state.cmp_staged.get(
                            tag, row["Value B"]
                        )
                        new_val = st.text_input(
                            "New value for B",
                            value=current_staged,
                            key=f"edit_{tag}",
                        )
                        staged_mark = (
                            " ✅ Staged"
                            if tag in st.session_state.cmp_staged
                            else ""
                        )
                        if st.button(
                            f"📌 Stage{staged_mark}",
        ...
        if not st.session_state.cmp_staged:
            st.markdown(
...
with st.sidebar:
    ...
    <div class="how-step"><div class="how-step-num">3</div>
        <div class="how-step-text">Expand tag → Edit → Stage</div></div>
    <div class="how-step"><div class="how-step-num">4</div>
        <div class="how-step-text">Apply &amp; Download modified File B</div></div>
    """, unsafe_allow_html=True)
